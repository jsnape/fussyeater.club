...
function validateCreateRecipeBody(body: CreateRecipeBody): ValidationError[] {
	...
	if (body.prepMinutes !== undefined && body.prepMinutes !== null) {
		if (typeof body.prepMinutes !== 'number' || !Number.isInteger(body.prepMinutes) || body.prepMinutes < 0) {
			errors.push({ field: 'prepMinutes', message: 'Prep minutes must be a non-negative integer' });
		}
	}

	if (body.cookMinutes !== undefined && body.cookMinutes !== null) {
		if (typeof body.cookMinutes !== 'number' || !Number.isInteger(body.cookMinutes) || body.cookMinutes < 0) {
			errors.push({ field: 'cookMinutes', message: 'Cook minutes must be a non-negative integer' });
		}
	}
	...
	if (!Array.isArray(body.ingredients) || body.ingredients.length === 0) {
		...
		for (let i = 0; i < body.ingredients.length; i++) {
			...
			if (!ing || typeof ing.ingredient !== 'string' || ing.ingredient.trim().length === 0) {
				errors.push({ field: `ingredients[${i}].ingredient`, message: 'Ingredient name is required' });
			}
		}
	}
	...
	if (body.type === 'full') {
		if (!Array.isArray(body.method) || body.method.length === 0) {
			errors.push({ field: 'method', message: 'At least one method step is required for full recipes' });
		} else {
			for (let i = 0; i < body.method.length; i++) {
				if (typeof body.method[i] !== 'string' || (body.method[i] as string).trim().length === 0) {
					errors.push({ field: `method[${i}]`, message: 'Method step must be a non-empty string' });
				}
			}
		}
	}
	...
	if (body.type === 'reference') {
		if (Array.isArray(body.method) && body.method.length > 0) {
			errors.push({ field: 'method', message: 'Reference recipes must not include method steps' });
		}
		const src = body.sourceReference as Record<string, unknown> | undefined;
		if (!src || (src.kind !== 'url' && src.kind !== 'book')) {
			errors.push({ field: 'sourceReference', message: 'Source reference with kind "url" or "book" is required for reference recipes' });
		} else {
			if (typeof src.label !== 'string' || src.label.trim().length === 0) {
				errors.push({ field: 'sourceReference.label', message: 'Source reference label is required' });
			}
			if (src.kind === 'url' && (typeof src.url !== 'string' || src.url.trim().length === 0)) {
				errors.push({ field: 'sourceReference.url', message: 'URL is required for URL source references' });
			}
		}
	}
...
export const POST: RequestHandler = async (event) => {
	...
	if (errors.length > 0) {
		...
		return jsonWithRequestId({ message: 'Validation failed', errors }, requestId, { status: 400 });
	}
	...
	try {
		...
		logInfo('recipes.create.success', requestId, {
			...
			visibility: recipe.visibility,
			slugCollision: recipe.id !== (body.title as string).trim().toLowerCase().replace(/\s+/g, '-')
		});
		...
		if (error instanceof Error && error.message === 'SLUG_COLLISION_EXHAUSTED') {
			...
			return jsonWithRequestId(
				{ message: 'Could not generate a unique URL for this recipe title. Please try a different title.' },
				requestId,
				{ status: 409 }
			);
		}
