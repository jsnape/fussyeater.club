...
describe('POST /api/recipes', () => {
    ...
    async function seedUserNoHousehold(pair: ReturnType<typeof createTestDbPair>): Promise<void> {
        await pair.first
            .prepare(
                "INSERT INTO users (id, email, name) VALUES ('user-2', 'user2@example.com', 'User2')"
            )
            .run();
        await pair.first
            .prepare(
    ...
    it('should create a full recipe and return 201', async () => {
        ...
        const response = await POST(
            makeEvent(pair.first, validFullRecipe, { cookie: 'session=sess-1' })
        );
